...
const TABLES = { news: 'news', links: 'quick_links', policies: 'policies', events: 'events', menu: 'menu_items', kb: 'kb_articles', owners: 'service_owners' };
...
const MAP = {
  ...
  menu: {
    toDb: r => ({ name_ar: r.ar, name_en: r.en, category: r.cat, price: Number(r.price) || 0,
      ...
      custom_options: r.options && r.options.length ? r.options : null,
  ...
  owners: {
    toDb: r => ({ service_ar: r.ar, service_en: r.en, owner_ar: r.ownerAR || '', owner_en: r.ownerEN || '',
      contact_email: r.contactEmail || null, contact_link: r.contactLink || null }),
    fromDb: r => ({ id: r.id, ar: r.service_ar, en: r.service_en, ownerAR: r.owner_ar, ownerEN: r.owner_en,
      contactEmail: r.contact_email, contactLink: r.contact_link }),
  },
};
...
export async function submitOrder({ branchId, requesterName, requesterNameAr, requesterNameEn, location, lines, paymentMethod }) {
  ...
  const { error: e2 } = await sb.from('order_items').insert(
    lines.map(l => ({
      ...
      qty: l.qty, sugar_level: l.sugar, milk: l.milk ?? null, mint: l.mint ?? null,
      custom_option_ar: l.optionAr ?? null, custom_option_en: l.optionEn ?? null, note: l.note,
